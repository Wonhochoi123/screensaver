...
local function update_music_display()
    mp.command_native_async({
        name = "subprocess", capture_stdout = true,
        args = { "/bin/sh", "-c",
            "printf '%s\\n' '{\"command\":[\"get_property\",\"metadata\"]}' | socat -t1 - UNIX-CONNECT:/tmp/ss_audio.sock 2>/dev/null" },
    }, function(ok, res)
        if ok and res and res.stdout and res.stdout ~= "" then
            ...
            if j and j.data and j.error == "success" then
                ...
                if not title then
                    mp.command_native_async({
                        name = "subprocess", capture_stdout = true,
                        args = { "/bin/sh", "-c",
                            "printf '%s\\n' '{\"command\":[\"get_property\",\"media-title\"]}' | socat -t1 - UNIX-CONNECT:/tmp/ss_audio.sock 2>/dev/null" },
                    }, function(ok2, res2)
                        if ok2 and res2 and res2.stdout and res2.stdout ~= "" then
                            ...
                            if j2 and j2.data and j2.error == "success" then
                                ...
                                music_ov.data = string.format(
                                    "{\\an7\\pos(%d,%d)\\fnMontserrat SemiBold\\fs%d\\bord0\\shad2\\1c&HFFFFFF&}♪  %s",
                                    pad, pad, fs, t)
                                music_ov:update()
                                return
                            end
                ...
                local line = title
                if artist and artist ~= "" then line = line .. "  ·  " .. artist end
                line = line:sub(1, 70)
                local w, h = refresh_display_size()
                local pad = math.floor(h * 0.025)
                local fs  = math.floor(h * 0.022 + 0.5)
